...
function setup_vendor() {
    ...
    export PRODUCTMK="$VALIDUS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$VALIDUS_ROOT"/vendor/validus/build/tools/smali/baksmali.jar
        export SMALIJAR="$VALIDUS_ROOT"/vendor/validus/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$VALIDUS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$VALIDUS_TARGET" >/dev/null; then
...
function extract() {
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$VALIDUS_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$VALIDUS_ROOT"/vendor/validus/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$VALIDUS_ROOT"/"$OUTDIR"/radio
